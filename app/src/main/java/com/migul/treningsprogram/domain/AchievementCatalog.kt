...
object AchievementCatalog {
    ...
    enum class Category(val label: String) {
        WORKOUTS("Workouts"), STREAKS("Streaks"), RECORDS("Records"), STRENGTH("Strength"),
        XP("XP"), BIG_SESSIONS("Big Sessions"), VARIETY("Variety"), COMBOS("Combos")
    }

    /** What lifetime stat (if any) the threshold applies to. */
    enum class Kind {
        WORKOUTS, STREAK, BEST_STREAK, PRS, SESSION_PRS, STRENGTH, XP,
    ...
    data class Meta(val kind: Kind, val threshold: Int, val comboTier: Tier? = null) {
        val category: Category = when (kind) {
            ...
            Kind.STRENGTH -> Category.STRENGTH
            Kind.XP -> Category.XP
        ...
        val tier: Tier = comboTier ?: when (kind) {
            ...
            // Brief 02: the threshold is now a 0-100 strength score, so rarity is pinned to the
            // tier ladder it represents — 20 = Beginner overall, 40 = Novice, 60 = Intermediate,
            // 80 = Advanced, 100 = Elite everywhere.
            Kind.STRENGTH -> tierBy(threshold, 20, 40, 70)
    ...
    private val NAMED: Map<String, Meta> = buildMap {
        ...
        // Brief 02: these nine keep their ids (they are persisted rows) but now read the
        // strength score, not the retired XP level.
        put("nine_lives", Meta(Kind.STRENGTH, 9)); put("one_for_eleven", Meta(Kind.STRENGTH, 11))
        put("battle_hardened", Meta(Kind.STRENGTH, 14)); put("seventeen_up", Meta(Kind.STRENGTH, 17))
        put("over_the_line", Meta(Kind.STRENGTH, 22)); put("forty_five_lives", Meta(Kind.STRENGTH, 45))
        put("diamond_level", Meta(Kind.STRENGTH, 60)); put("the_overlord", Meta(Kind.STRENGTH, 80))
        put("the_transcendent", Meta(Kind.STRENGTH, 90))
        // XP evocative names
    ...
    fun metaFor(id: String): Meta? {
        ...
        return when {
            ...
            // Brief 02: the `level_` PREFIX deliberately survives the retirement of the XP level.
            // These ids are persisted rows carrying the user's unlock timestamps, and
            // `ensureAchievementsSeeded` hard-deletes any row whose id is no longer in the seed
            // list. Renaming `level_40` to `strength_40` would silently destroy an earned
            // achievement seconds after the app next opens. The id is storage; only its MEANING
            // moved — from XP level to strength score.
            id.startsWith("level_") -> num("level_")?.let { Meta(Kind.STRENGTH, it) }
     ...
     */
    fun progressFor(id: String, stats: UserStats, strengthScore: Int): Pair<Int, Int>? {
        ...
        val current = when (meta.kind) {
            ...
            // Brief 02: was stats.level (XP). The strength score is a live, derived value rather
            // than a column on UserStats, which is why it arrives as a parameter.
            Kind.STRENGTH -> strengthScore
    ...
    val CATEGORY_ORDER: List<Category> = listOf(
        Category.WORKOUTS, Category.STREAKS, Category.RECORDS, Category.STRENGTH,
    ...
    fun nextUp(
        ...
        achievements
            ...
            .mapNotNull { a ->
                val p = progressFor(a.id, stats, strengthScore) ?: return@mapNotNull null
